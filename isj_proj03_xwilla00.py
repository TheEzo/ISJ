...
def first_task():
    ...
    # inversion using zip
    plur2sing = {p: s for p, s in zip(plural, singular)}  # vase reseni
    return plur2sing == {'geese': 'goose', 'men': 'man', 'children': 'child'}


def second_task():
    word2vect = {'king': [3, 1], 'queen': [6, 3], 'uncle': [4, 3], 'aunt': [8, 9]}
    vect2word = {(3, 1): 'king', (6, 3): 'queen', (4, 3): 'uncle', (8, 9): 'aunt'}  # vase reseni
...
class MyList(list):
    def __hash__(self):
        return sum(c for c in self)  # nahradte za vase reseni
